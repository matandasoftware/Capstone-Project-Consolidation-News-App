...
def add_book():
    ...
    print("\n--- ADD NEW BOOK ---")
    try:
        book_id = validate_positive_integer("Enter book ID: ")
        title = validate_non_empty_string("Enter book title: ")
        author_id = validate_positive_integer("Enter author ID: ")
        ...
        with get_database_connection() as conn:
            ...
            if not cursor.fetchone():
                ...
                print("Please add the author first or use an existing author ID.")
                return
...
def update_book():
    ...
    print("\n--- UPDATE BOOK ---")
    try:
        book_id = validate_positive_integer("Enter book ID to update: ")
        with get_database_connection() as conn:
            cursor = conn.cursor()
            ...
            if choice == 1:
                ...
                new_author_id = validate_positive_integer("Enter new author ID: ")
                # Verify new author exists
                cursor.execute('SELECT ID FROM AUTHOR WHERE ID = ?', (new_author_id,))
                if not cursor.fetchone():
                    print(f"Error: Author with ID {new_author_id} does not exist.")
                    return
...
def delete_book():
    ...
    print("\n--- DELETE BOOK ---")
    try:
        book_id = validate_positive_integer("Enter book ID to delete: ")
        with get_database_connection() as conn:
            cursor = conn.cursor()
